    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", paddingBottom: "16px", borderBottom: "0.5px solid rgba(255,255,255,0.10)" }}>
        <div>
          ...
          <p style={{ fontSize: "12px", color: "#B5BAC2", marginTop: "5px", fontFamily: "'Barlow', sans-serif", textTransform: "none", letterSpacing: "normal", fontWeight: 400 }}>Track all purchase orders to the China factory</p>
        </div>
      ...
      <div style={{ display: "flex", borderBottom: "0.5px solid rgba(255,255,255,0.10)" }}>
        {(["all", "draft", "placed", "in_production", "shipped", "received"] as const).map(s => (
          <button key={s} onClick={() => setFilterStatus(s)} style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "11px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "10px 16px", cursor: "pointer", border: "none", background: "transparent", color: filterStatus === s ? "#fff" : "#8B919A", borderBottom: filterStatus === s ? "2px solid #A91E22" : "2px solid transparent", marginBottom: "-1px" }}>
      ...
      {/* PO List */}
      {loading ? (
        <div style={{ padding: "60px", textAlign: "center", color: "#787E87", fontFamily: "'Barlow Condensed', sans-serif", fontSize: "14px", letterSpacing: "0.1em", textTransform: "uppercase" }}>Loading POs...</div>
      ) : filtered.length === 0 ? (
        <div style={{ background: "#2E343C", border: "0.5px solid rgba(255,255,255,0.10)", padding: "60px 20px", textAlign: "center" }}>
          ...
          <p style={{ fontSize: "13px", color: "#666C75", fontFamily: "'Barlow', sans-serif", margin: "0 0 20px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {filtered.map(po => {
            ...
            return (
              <div key={po.id} style={{ background: "#2E343C", border: "0.5px solid rgba(255,255,255,0.10)" }}>
                ...
                <div onClick={() => setExpandedPO(expanded ? null : po.id)} style={{ padding: "16px 20px", display: "flex", alignItems: "center", gap: "16px", cursor: "pointer" }}>
                  <div style={{ flex: "0 0 140px" }}>
                    ...
                    <p style={{ fontSize: "11px", color: "#8B919A", fontFamily: "'Barlow', sans-serif", margin: "2px 0 0" }}>{po.factory_name}</p>
                  </div>
                  ...
                  <div style={{ flex: 1, display: "flex", gap: "24px" }}>
                    <div>
                      <p style={{ fontSize: "9px", color: "#787E87", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.12em", textTransform: "uppercase", margin: "0 0 2px" }}>Order Date</p>
                      <p style={{ fontSize: "12px", color: "#AAA", fontFamily: "'Barlow', sans-serif", margin: 0 }}>{formatDate(po.order_date)}</p>
                    </div>
                    {po.expected_ship_date && (
                      <div>
                        <p style={{ fontSize: "9px", color: "#787E87", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.12em", textTransform: "uppercase", margin: "0 0 2px" }}>Expected Ship</p>
                  ...
                  <div style={{ flex: "0 0 120px", textAlign: "right" }}>
                    ...
                    <p style={{ fontSize: "11px", color: "#8B919A", fontFamily: "'Barlow', sans-serif", margin: "2px 0 0" }}>
                      {po.deposit_paid_date && !po.final_payment_paid_date ? "50% paid" : po.final_payment_paid_date ? "Paid in full" : "Deposit pending"}
                    </p>
                  </div>

                  <ChevronDown size={16} color="#787E87" style={{ transform: expanded ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.15s", flexShrink: 0 }} />
                </div>
                ...
                {expanded && (
                  <div style={{ borderTop: "0.5px solid rgba(255,255,255,0.08)", background: "#2B3038" }}>
                    ...
                    <div style={{ padding: "16px 20px", borderBottom: "0.5px solid rgba(255,255,255,0.06)" }}>
                      <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "#8B919A", marginBottom: "12px" }}>Payment Timeline</p>
                      <div style={{ display: "flex", gap: "12px" }}>
                        {[
                          ...
                          <div key={payment.label} style={{ background: "#2E343C", border: `0.5px solid ${payment.paid ? "rgba(90,158,90,0.3)" : "rgba(169,30,34,0.2)"}`, padding: "12px 16px", flex: 1 }}>
                            ...
                            <p style={{ fontSize: "11px", color: "#8B919A", fontFamily: "'Barlow', sans-serif", margin: 0 }}>{payment.paid ? `Paid ${formatDate(payment.paidDate!)}` : payment.due}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div style={{ padding: "16px 20px", borderBottom: "0.5px solid rgba(255,255,255,0.06)" }}>
                      ...
                      >
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                          ...
                          <ChevronRight size={14} color="#787E87" style={{ transform: itemsExpanded ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.15s" }} />
                        </div>
                      </div>

                      {itemsExpanded && (
                        po.items.length === 0 ? (
                          <p style={{ fontSize: "12px", color: "#666C75", fontFamily: "'Barlow', sans-serif", fontStyle: "italic" }}>No line items recorded for this PO</p>
                        ) : (
                            ...
                            <tbody>
                              {po.items.map(item => (
                                <tr key={item.id}>
                                  ...
                                  <td style={{ padding: "8px 12px", fontSize: "13px", color: "#E0E2E6", fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, borderBottom: "0.5px solid rgba(255,255,255,0.04)" }}>{item.quantity}</td>
                              ...
                              <tr>
                                <td colSpan={4} style={{ padding: "10px 12px", fontFamily: "'Barlow Condensed', sans-serif", fontSize: "11px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#8B919A", textAlign: "right" }}>PO Total</td>
                    ...
                    {po.notes && <p style={{ fontSize: "12px", color: "#8B919A", fontFamily: "'Barlow', sans-serif", padding: "0 20px 16px", fontStyle: "italic" }}>Note: {po.notes}</p>}
