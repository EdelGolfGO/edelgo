...
export default function DashboardPage() {
  ...
  async function loadDashboard() {
    ...
    for (const inv of invoices) {
      ...
      if (!inv.deposit_paid_date && depositDue < today) {
        overdue.push({
          type: inv.invoice_type === "factory" ? "Factory Deposit Overdue" : "Distributor Deposit Overdue",
          ref: inv.invoice_number,
          amount: inv.total_amount * 0.5,
          days: Math.ceil((today.getTime() - depositDue.getTime()) / (1000 * 60 * 60 * 24))
        })
      }
    }
  ...
  const PO_STATUS_COLORS: Record<string, string> = {
    draft: "#B5BAC2", placed: "#6A9CC8", in_production: "#C4A93A",
    shipped: "#7AAB6A", received: "#5A9E5A", cancelled: "#A91E22",
  }

  const INV_STATUS_COLORS: Record<string, string> = {
    pending: "#B5BAC2", deposit_received: "#6A9CC8", shipped: "#C4A93A",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "10px" }}>
        {[
          { label: "Active POs", value: loading ? "—" : stats.activePOs.toString(), color: "#fff", top: "#3A3F47", href: "/operations/pos" },
          { label: "Pending Invoices", value: loading ? "—" : stats.pendingInvoices.toString(), color: stats.pendingInvoices > 0 ? "#C4A93A" : "#5A9E5A", top: "#3A3F47", href: "/operations/invoices" },
          ...
          <div key={stat.label} onClick={() => router.push(stat.href)}
            style={{ background: "#2E343C", border: "0.5px solid rgba(255,255,255,0.10)", borderTop: `2px solid ${stat.top}`, padding: "18px 20px", cursor: "pointer" }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "10px" }}>
        {[
          ...
          <div key={stat.label} onClick={() => router.push(stat.href)}
            style={{ background: "#2E343C", border: "0.5px solid rgba(255,255,255,0.10)", borderTop: "2px solid #3A3F47", padding: "16px 20px", cursor: "pointer" }}
            onMouseEnter={e => e.currentTarget.style.background = "#262B30"}
            onMouseLeave={e => e.currentTarget.style.background = "#2E343C"}
          >
            <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 600, letterSpacing: "0.16em", textTransform: "uppercase", color: "#B5BAC2", marginBottom: "6px" }}>{stat.label}</p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
        ...
        <div style={{ background: "#2E343C", border: "0.5px solid rgba(255,255,255,0.10)" }}>
          <div style={{ padding: "14px 18px", borderBottom: "0.5px solid rgba(255,255,255,0.08)", background: "#262B32", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "11px", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "#9BA0A8" }}>Recent Purchase Orders</span>
          ...
          {loading ? (
            <div style={{ padding: "30px", textAlign: "center", color: "#666C75", fontSize: "12px", fontFamily: "'Barlow', sans-serif" }}>Loading...</div>
          ) : recentPOs.length === 0 ? (
            <div style={{ padding: "30px", textAlign: "center", color: "#666C75", fontSize: "12px", fontFamily: "'Barlow', sans-serif" }}>No POs yet — <span style={{ color: "#A91E22", cursor: "pointer" }} onClick={() => router.push("/operations/pos")}>create one</span></div>
          ) : (
            recentPOs.map(po => (
              <div key={po.id} onClick={() => router.push("/operations/pos")}
                style={{ display: "flex", alignItems: "center", gap: "12px", padding: "12px 18px", borderBottom: "0.5px solid rgba(255,255,255,0.05)", cursor: "pointer" }}
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: "11px", color: "#8B919A", fontFamily: "'Barlow', sans-serif", margin: "2px 0 0" }}>{po.factory_name} · {formatDate(po.order_date)}</p>
                </div>
                <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: PO_STATUS_COLORS[po.status] || "#B5BAC2", background: `${PO_STATUS_COLORS[po.status]}18` || "rgba(136,136,136,0.1)", padding: "2px 8px" }}>
                  {po.status.replace("_", " ")}
                </span>
                <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "14px", fontWeight: 700, color: "#E0E2E6" }}>${po.total_amount?.toLocaleString()}</span>
              </div>
            ))
          )}
        </div>
        ...
        <div style={{ background: "#2E343C", border: "0.5px solid rgba(255,255,255,0.10)" }}>
          <div style={{ padding: "14px 18px", borderBottom: "0.5px solid rgba(255,255,255,0.08)", background: "#262B32", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "11px", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "#9BA0A8" }}>Recent Invoices</span>
          ...
          {loading ? (
            ...
            <div style={{ padding: "30px", textAlign: "center", color: "#666C75", fontSize: "12px", fontFamily: "'Barlow', sans-serif" }}>No invoices yet — <span style={{ color: "#5A9E5A", cursor: "pointer" }} onClick={() => router.push("/operations/invoices")}>create one</span></div>
          ) : (
            recentInvoices.map(inv => {
              const isFactory = inv.invoice_type === "factory"
              const accentColor = isFactory ? "#A91E22" : "#5A9E5A"
              return (
                <div key={inv.id} onClick={() => router.push("/operations/invoices")}
                  style={{ display: "flex", alignItems: "center", gap: "12px", padding: "12px 18px", borderBottom: "0.5px solid rgba(255,255,255,0.05)", cursor: "pointer" }}
                  onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.02)"}
                  onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                >
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontSize: "11px", color: "#8B919A", fontFamily: "'Barlow', sans-serif", margin: "2px 0 0" }}>{inv.dealer_name} · {formatDate(inv.invoice_date)}</p>
                  </div>
                  <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: INV_STATUS_COLORS[inv.status] || "#B5BAC2", padding: "2px 8px", background: `${INV_STATUS_COLORS[inv.status] || "#B5BAC2"}18` }}>
                    {inv.status.replace("_", " ")}
                  </span>
                  <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "14px", fontWeight: 700, color: "#E0E2E6" }}>${inv.total_amount?.toLocaleString()}</span>
                </div>
              )
            })
          )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "10px" }}>
        {[
          ...
          <div key={link.label} onClick={() => router.push(link.href)}
            style={{ background: "#2E343C", border: "0.5px solid rgba(255,255,255,0.08)", borderTop: `2px solid ${link.color}`, padding: "14px 16px", cursor: "pointer" }}
            onMouseEnter={e => e.currentTarget.style.background = "#262B30"}
            onMouseLeave={e => e.currentTarget.style.background = "#2E343C"}
          >
            ...
            <p style={{ fontSize: "11px", color: "#8B919A", fontFamily: "'Barlow', sans-serif", margin: 0 }}>{link.desc}</p>
          </div>
        ))}
      </div>
