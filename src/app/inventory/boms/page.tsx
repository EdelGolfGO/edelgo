...
export default function BomsPage() {
  ...
  async function loadAll() {
    ...
    const [{ data: skuData }, { data: allSkuData }, { data: supplierData }, { data: headersData }, { data: itemsData }, { data: settingsData }] = await Promise.all([
      supabase.from("skus").select("id, sku_code, name, unit_cost, msrp, sku_type, product:products(name, category)").eq("sku_type", "built_product").order("sku_code"),
    ...
    if (skuData && headersData && itemsData) {
      const excludedCategories = ["apparel", "accessory"]
      const merged: FinishedSKU[] = skuData
        .filter((sku: any) => !excludedCategories.includes(sku.product?.category))
        .map((sku: any) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          <div style={{ position: "relative" }}>
            ...
            <input placeholder="Search built products..." value={search} onChange={e => setSearch(e.target.value)}
              style={{ background: "#262B32", border: "0.5px solid rgba(255,255,255,0.10)", color: "#fff", padding: "8px 14px 8px 30px", fontSize: "12px", fontFamily: "'Barlow', sans-serif", outline: "none", width: "220px" }} />
          </div>
      ...
      <div style={{ display: "flex", borderBottom: "0.5px solid rgba(255,255,255,0.10)" }}>
        {[
          ...
          <button key={t.key} onClick={() => setDashboardTab(t.key as any)}
            style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "11px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "10px 16px", cursor: "pointer", border: "none", background: "transparent", color: dashboardTab === t.key ? "#fff" : "#8B919A", borderBottom: dashboardTab === t.key ? "2px solid #A91E22" : "2px solid transparent", marginBottom: "-1px" }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {!loading && finishedSkus.length === 0 && (
        <div style={{ background: "#2E343C", border: "0.5px solid rgba(255,255,255,0.08)", borderLeft: "3px solid #C4A93A", padding: "16px 20px" }}>
          ...
          <p style={{ fontSize: "12px", color: "#9BA0A8", fontFamily: "'Barlow', sans-serif", margin: 0 }}>
            Go to <strong style={{ color: "#aaa" }}>Inventory → SKUs</strong> and mark SKUs as "Built Product" to have them appear here.
          </p>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {filteredSkus.map(sku => {
            ...
            return (
              <div key={sku.id} style={{ background: "#2E343C", border: "0.5px solid rgba(255,255,255,0.10)", borderLeft: `3px solid ${hasBom ? "#5A9E5A" : "#C4A93A"}` }}>
                <div onClick={() => setExpanded(isExpanded ? null : sku.id)}
                  ...
                  <div style={{ flex: "0 0 180px" }}>
                    ...
                    <p style={{ fontSize: "11px", color: "#8B919A", fontFamily: "'Barlow', sans-serif", margin: "2px 0 0" }}>{sku.name}</p>
                  </div>
                  ...
                  <div style={{ flex: 1, display: "flex", gap: "24px" }}>
                    {hasBom && (
                      <>
                        <div>
                          <p style={{ fontSize: "9px", color: "#787E87", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.12em", textTransform: "uppercase", margin: "0 0 2px" }}>Materials</p>
                          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "14px", fontWeight: 700, color: "#E0E2E6", margin: 0 }}>${materialCost.toFixed(2)}</p>
                        </div>
                        <div>
                          <p style={{ fontSize: "9px", color: "#787E87", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.12em", textTransform: "uppercase", margin: "0 0 2px" }}>Labor</p>
                          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "14px", fontWeight: 700, color: "#E0E2E6", margin: 0 }}>${laborCost.toFixed(2)}</p>
                        </div>
                        <div>
                          <p style={{ fontSize: "9px", color: "#787E87", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.12em", textTransform: "uppercase", margin: "0 0 2px" }}>COGS/Unit</p>
                          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "14px", fontWeight: 700, color: "#E8C84A", margin: 0 }}>${totalCogs.toFixed(2)}</p>
                        </div>
                        {sku.msrp && sku.msrp > 0 && (
                          <div>
                            <p style={{ fontSize: "9px", color: "#787E87", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.12em", textTransform: "uppercase", margin: "0 0 2px" }}>MSRP</p>
                            <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "14px", fontWeight: 700, color: "#E0E2E6", margin: 0 }}>${sku.msrp.toFixed(2)}</p>
                          </div>
                        )}
                        {margin && (
                          <div>
                            <p style={{ fontSize: "9px", color: "#787E87", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.12em", textTransform: "uppercase", margin: "0 0 2px" }}>Margin</p>
                  ...
                  <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                    ...
                    {hasBom && (
                      <button onClick={e => { e.stopPropagation(); setDeleteBomConfirm({ skuId: sku.id, headerId: sku.bom_header!.id, skuCode: sku.sku_code }) }}
                        style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "#8B919A", background: "transparent", border: "1px solid #666C75", padding: "5px 10px", cursor: "pointer" }}
                ...
                {isExpanded && hasBom && (
                  <div style={{ borderTop: "0.5px solid rgba(255,255,255,0.08)", padding: "16px 20px", background: "#2B3038" }}>
                    <div style={{ display: "flex", gap: "24px", marginBottom: "12px", paddingBottom: "12px", borderBottom: "0.5px solid rgba(255,255,255,0.06)" }}>
                      <div>
                        <p style={{ fontSize: "9px", color: "#787E87", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.12em", textTransform: "uppercase", margin: "0 0 2px" }}>Est. Build Time / Product</p>
                        <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "13px", fontWeight: 700, color: "#B5BAC2", margin: 0 }}>{sku.bom_header?.labor_minutes || 0} min</p>
                      </div>
                      <div>
                        <p style={{ fontSize: "9px", color: "#787E87", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.12em", textTransform: "uppercase", margin: "0 0 2px" }}>Labor Rate</p>
                        <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "13px", fontWeight: 700, color: "#B5BAC2", margin: 0 }}>${sku.bom_header?.labor_rate_per_hour || 0}/hr</p>
                      </div>
                      <div>
                        <p style={{ fontSize: "9px", color: "#787E87", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.12em", textTransform: "uppercase", margin: "0 0 2px" }}>Labor Cost/Unit</p>
                        <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "13px", fontWeight: 700, color: "#E8C84A", margin: 0 }}>${laborCost.toFixed(2)}</p>
                      </div>
                    </div>

                    <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "#8B919A", marginBottom: "12px" }}>Components</p>
                    <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                      {(sku.bom_items || []).map(item => (
                        <div key={item.id} style={{ display: "grid", gridTemplateColumns: "160px 1fr 90px 80px 80px 100px 40px", gap: "12px", alignItems: "center", padding: "8px 12px", background: "#2E343C", border: "0.5px solid rgba(255,255,255,0.05)" }}>
                          ...
                          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "12px", color: "#9BA0A8", margin: 0, textAlign: "right" }}>
                            ${(item.unit_cost || item.component?.unit_cost || item.component?.cost_per_uom || 0).toFixed(item.component?.sku_type === "consumable" ? 4 : 2)}
                          </p>
                          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "12px", fontWeight: 700, color: "#E0E2E6", margin: 0, textAlign: "right" }}>${((item.unit_cost || item.component?.unit_cost || item.component?.cost_per_uom || 0) * item.quantity).toFixed(2)}</p>
                          <button onClick={() => setDeleteConfirm({ itemId: item.id, name: item.component?.name || "component" })}
                            style={{ background: "none", border: "none", color: "#666C75", cursor: "pointer", padding: "2px" }}>
                            <Trash2 size={12} />
                          </button>
                        </div>
                      ))}
                    ...
                    <div style={{ display: "flex", justifyContent: "flex-end", gap: "32px", marginTop: "12px", paddingTop: "12px", borderTop: "0.5px solid rgba(255,255,255,0.08)" }}>
                      <div style={{ textAlign: "right" }}>
                        <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "9px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#787E87", margin: "0 0 2px" }}>Materials</p>
                        <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "16px", fontWeight: 700, color: "#E0E2E6", margin: 0 }}>${materialCost.toFixed(2)}</p>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "9px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#787E87", margin: "0 0 2px" }}>Labor</p>
                        <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "16px", fontWeight: 700, color: "#E0E2E6", margin: 0 }}>${laborCost.toFixed(2)}</p>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "9px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#787E87", margin: "0 0 2px" }}>Total COGS/Unit</p>
                      ...
                      {sku.msrp && sku.msrp > 0 && totalCogs > 0 && (
                        <div style={{ textAlign: "right" }}>
                          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "9px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#787E87", margin: "0 0 2px" }}>Gross Margin</p>
      ...
      {modal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: "20px" }} onClick={() => setModal(false)}>
          <div style={{ background: "#2B3038", border: "0.5px solid rgba(255,255,255,0.10)", borderTop: "2px solid #A91E22", width: "100%", maxWidth: "960px", maxHeight: "90vh", display: "flex", flexDirection: "column" }} onClick={e => e.stopPropagation()}>

            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px", borderBottom: "0.5px solid rgba(255,255,255,0.08)", background: "#20242A", flexShrink: 0 }}>
              <div>
            ...
            {modalStep === "select_product" && (
              <div style={{ padding: "20px 24px", overflowY: "auto", flex: 1 }}>
                <p style={{ fontSize: "13px", color: "#9BA0A8", fontFamily: "'Barlow', sans-serif", marginBottom: "16px" }}>
                  Select a built product to create or edit its BoM. Only SKUs marked as "Built Product" appear here.
                </p>
                <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                  {finishedSkus.length === 0 ? (
                    <div style={{ padding: "32px", textAlign: "center", color: "#787E87", fontSize: "13px", fontFamily: "'Barlow', sans-serif" }}>
                      No built products found. Go to SKUs and mark some as "Built Product" first.
                    </div>
                  ) : (
                    finishedSkus.map(sku => (
                      <div
                        ...
                        style={{ display: "flex", alignItems: "center", gap: "16px", padding: "12px 16px", background: "#2E343C", border: "0.5px solid rgba(255,255,255,0.06)", cursor: "pointer", transition: "border-color 0.1s" }}
                      ...
                      >
                        <div style={{ flex: 1 }}>
                          ...
                          <p style={{ fontSize: "12px", color: "#B5BAC2", fontFamily: "'Barlow', sans-serif", margin: "2px 0 0" }}>{sku.name}</p>
                        </div>
                        {sku.msrp && (
                          <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "12px", color: "#8B919A" }}>MSRP ${sku.msrp.toFixed(2)}</span>
                        )}
                        <span style={{
                        ...
                        <span style={{ color: "#787E87", fontSize: "14px" }}>→</span>
                      </div>
                    ))
                  )}
                </div>
            ...
            {modalStep === "add_components" && selectedSku && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", flex: 1, overflow: "hidden" }}>
                ...
                <div style={{ borderRight: "0.5px solid rgba(255,255,255,0.08)", display: "flex", flexDirection: "column" }}>
                  <div style={{ padding: "16px", borderBottom: "0.5px solid rgba(255,255,255,0.08)", background: "#20242A", flexShrink: 0 }}>
                    <div style={{ display: "flex", gap: "6px", marginBottom: "10px" }}>
                      <button onClick={() => setPickerTab("component")}
                        style={{ flex: 1, fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", padding: "7px", cursor: "pointer", border: "none", background: pickerTab === "component" ? "rgba(196,169,58,0.18)" : "#23282E", color: pickerTab === "component" ? "#C4A93A" : "#8B919A" }}>
                        Components ({componentTypeCount})
                      </button>
                      <button onClick={() => setPickerTab("consumable")}
                        style={{ flex: 1, fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", padding: "7px", cursor: "pointer", border: "none", background: pickerTab === "consumable" ? "rgba(106,156,200,0.18)" : "#23282E", color: pickerTab === "consumable" ? "#6A9CC8" : "#8B919A" }}>
                    ...
                    <div style={{ position: "relative" }}>
                      ...
                      <input placeholder={`Search ${pickerTab === "component" ? "components" : "consumables"}...`} value={componentSearch} onChange={e => setComponentSearch(e.target.value)}
                        style={{ width: "100%", background: "#23282E", border: "0.5px solid rgba(255,255,255,0.12)", color: "#fff", padding: "7px 10px 7px 26px", fontSize: "12px", fontFamily: "'Barlow', sans-serif", outline: "none", boxSizing: "border-box" as const }} />
                    </div>
                  </div>
                  <div style={{ overflowY: "auto", flex: 1, padding: "8px" }}>
                    {filteredComponents.length === 0 ? (
                      <div style={{ padding: "32px", textAlign: "center", color: "#787E87", fontSize: "12px", fontFamily: "'Barlow', sans-serif" }}>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={{ width: "20px", height: "20px", background: alreadyAdded ? "#A91E22" : "#666C75", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  <div style={{ padding: "16px", borderBottom: "0.5px solid rgba(255,255,255,0.08)", background: "#20242A", flexShrink: 0 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
                      <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#8B919A", margin: 0 }}>
                        BoM for {selectedSku.sku_code} — {bomLines.length} line{bomLines.length !== 1 ? "s" : ""}
                      </p>
                      <button onClick={() => setModalStep("select_product")}
                        style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "#8B919A", background: "transparent", border: "1px solid #666C75", padding: "4px 10px", cursor: "pointer" }}>
                        ← Change
                      </button>
                    </div>
                  ...
                  <div style={{ overflowY: "auto", flex: 1, padding: "8px" }}>
                    {bomLines.length === 0 ? (
                      <div style={{ padding: "40px", textAlign: "center", color: "#666C75", fontSize: "13px", fontFamily: "'Barlow', sans-serif" }}>
                        No components yet.<br />Click items on the left to add them.
                      </div>
                    ) : (
                      bomLines.map(line => (
                        <div key={line.component_sku_id} style={{ background: "#2E343C", border: "0.5px solid rgba(255,255,255,0.06)", padding: "10px 12px", marginBottom: "6px" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "8px" }}>
                            <div>
                              ...
                              <p style={{ fontSize: "11px", color: "#B5BAC2", fontFamily: "'Barlow', sans-serif", margin: "2px 0 0" }}>{line.sku?.name}</p>
                            </div>
                            <button onClick={() => removeLine(line.component_sku_id)} style={{ background: "none", border: "none", color: "#666C75", cursor: "pointer", padding: "2px" }}>
                              <X size={14} />
                            </button>
                          </div>
                          ...
                          <div style={{ marginTop: "6px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            ...
                            <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "11px", fontWeight: 700, color: "#E0E2E6", whiteSpace: "nowrap" }}>
                  ...
                  <div style={{ padding: "16px", borderTop: "0.5px solid rgba(255,255,255,0.08)", background: "#20242A", flexShrink: 0 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                      <div style={{ display: "flex", gap: "20px" }}>
                        <div>
                          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "9px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#787E87", margin: "0 0 2px" }}>Materials</p>
                          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "16px", fontWeight: 700, color: "#E0E2E6", margin: 0 }}>${getMaterialCost().toFixed(2)}</p>
                        </div>
                        <div>
                          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "9px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#787E87", margin: "0 0 2px" }}>Labor</p>
                          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "16px", fontWeight: 700, color: "#E0E2E6", margin: 0 }}>${getLaborCost().toFixed(2)}</p>
                        </div>
                        <div>
                          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "9px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#787E87", margin: "0 0 2px" }}>Total COGS/Unit</p>
                      ...
                      {selectedSku.msrp && selectedSku.msrp > 0 && getTotalCogs() > 0 && (
                        <div style={{ textAlign: "right" }}>
                          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "9px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#787E87", margin: "0 0 2px" }}>vs MSRP ${selectedSku.msrp.toFixed(2)}</p>
                    ...
                    <button onClick={handleSaveBom} disabled={saving || bomLines.length === 0}
                      style={{ width: "100%", fontFamily: "'Barlow Condensed', sans-serif", fontSize: "13px", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#fff", background: saving || bomLines.length === 0 ? "#666C75" : "#A91E22", border: "none", padding: "11px", cursor: saving ? "not-allowed" : "pointer" }}>
      ...
      {deleteConfirm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 400 }} onClick={() => setDeleteConfirm(null)}>
          <div style={{ background: "#2B3038", border: "0.5px solid rgba(255,255,255,0.10)", borderTop: "2px solid #A91E22", padding: "32px", width: "380px" }} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize: "13px", color: "#B5BAC2", fontFamily: "'Barlow', sans-serif", margin: "0 0 24px" }}>Remove <strong style={{ color: "#fff" }}>{deleteConfirm.name}</strong> from this BoM?</p>
            <div style={{ display: "flex", gap: "10px" }}>
              <button onClick={() => setDeleteConfirm(null)}
                style={{ flex: 1, fontFamily: "'Barlow Condensed', sans-serif", fontSize: "12px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#B5BAC2", background: "transparent", border: "1px solid #666C75", padding: "10px", cursor: "pointer" }}>Cancel</button>
      ...
      {deleteBomConfirm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 400 }} onClick={() => setDeleteBomConfirm(null)}>
          <div style={{ background: "#2B3038", border: "0.5px solid rgba(255,255,255,0.10)", borderTop: "2px solid #A91E22", padding: "32px", width: "420px" }} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize: "12px", color: "#8B919A", fontFamily: "'Barlow', sans-serif", margin: "0 0 24px" }}>
