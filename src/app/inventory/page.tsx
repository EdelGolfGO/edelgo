    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", paddingBottom: "16px", borderBottom: "0.5px solid rgba(255,255,255,0.10)" }}>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          <button onClick={() => router.push("/inventory/forecast")} style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "12px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#B5BAC2", background: "transparent", border: "1px solid #666C75", padding: "8px 16px", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "10px" }}>
        {[
          { label: "Total SKUs", value: inventory.length.toString(), color: "#fff", top: "#3A3F47" },
          { label: "Critical (Out/Near 0)", value: criticalCount.toString(), color: criticalCount > 0 ? "#A91E22" : "#5A9E5A", top: criticalCount > 0 ? "#A91E22" : "#3A3F47" },
          { label: "Low Stock", value: lowCount.toString(), color: lowCount > 0 ? "#C4A93A" : "#5A9E5A", top: lowCount > 0 ? "#C4A93A" : "#3A3F47" },
          { label: "Inventory Value", value: `$${Math.round(totalValue).toLocaleString()}`, color: "#6A9CC8", top: "#3A3F47" },
        ].map(stat => (
      ...
      <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
        <div style={{ display: "flex", borderBottom: "0.5px solid rgba(255,255,255,0.10)" }}>
          {(["all", "critical", "low", "healthy"] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "11px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "10px 14px", cursor: "pointer", border: "none", background: "transparent", color: filter === f ? "#fff" : "#8B919A", borderBottom: filter === f ? "2px solid #A91E22" : "2px solid transparent", marginBottom: "-1px" }}>
        ...
        <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}
          style={{ background: "#262B32", border: "0.5px solid rgba(255,255,255,0.10)", color: categoryFilter !== "all" ? "#fff" : "#B5BAC2", padding: "8px 12px", fontSize: "12px", fontFamily: "'Barlow', sans-serif", outline: "none", cursor: "pointer" }}>
        ...
        <input
          ...
          style={{ marginLeft: "auto", background: "#262B32", border: "0.5px solid rgba(255,255,255,0.10)", color: "#fff", padding: "8px 14px", fontSize: "12px", fontFamily: "'Barlow', sans-serif", outline: "none", width: "220px" }}
        />
      </div>

      {/* Inventory Table */}
      {loading ? (
        <div style={{ padding: "60px", textAlign: "center", color: "#787E87", fontFamily: "'Barlow Condensed', sans-serif", fontSize: "14px", letterSpacing: "0.1em", textTransform: "uppercase" }}>Loading inventory...</div>
      ) : (
            ...
            <thead>
              <tr style={{ background: "#262B32" }}>
                {["SKU", "Product", "Type", "Category", "On Hand", "Reserved", "Available", "On Order", "Min", "Max", "Status", ""].map(h => (
                  <th key={h} style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "9px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#8B919A", padding: "10px 14px", textAlign: "left", borderBottom: "0.5px solid rgba(255,255,255,0.08)" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(item => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: "10px 14px", fontSize: "13px", color: "#E0E2E6", fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, borderBottom: "0.5px solid rgba(255,255,255,0.04)" }}>{item.qty_on_hand}</td>
                    <td style={{ padding: "10px 14px", fontSize: "12px", color: "#9BA0A8", fontFamily: "'Barlow Condensed', sans-serif", borderBottom: "0.5px solid rgba(255,255,255,0.04)" }}>{item.qty_reserved}</td>
                    <td style={{ padding: "10px 14px", borderBottom: "0.5px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <td style={{ padding: "10px 14px", fontSize: "12px", color: item.qty_on_order > 0 ? "#6A9CC8" : "#787E87", fontFamily: "'Barlow Condensed', sans-serif", fontWeight: item.qty_on_order > 0 ? 700 : 400, borderBottom: "0.5px solid rgba(255,255,255,0.04)" }}>{item.qty_on_order}</td>
                    ...
                    <td style={{ padding: "10px 14px", fontSize: "12px", color: "#8B919A", fontFamily: "'Barlow Condensed', sans-serif", borderBottom: "0.5px solid rgba(255,255,255,0.04)" }}>{item.max_stock}</td>
                    <td style={{ padding: "10px 14px", borderBottom: "0.5px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <td style={{ padding: "10px 14px", borderBottom: "0.5px solid rgba(255,255,255,0.04)" }}>
                      <button onClick={() => openEdit(item)} style={{ background: "none", border: "none", color: "#787E87", cursor: "pointer", padding: "2px", display: "flex" }} title="Edit min/max stock levels">
                        <Pencil size={13} />
                      </button>
                    </td>
                  </tr>
      ...
      {editModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: "20px" }} onClick={() => setEditModal(null)}>
          <div style={{ background: "#2B3038", border: "0.5px solid rgba(255,255,255,0.10)", borderTop: "2px solid #6A9CC8", width: "100%", maxWidth: "440px" }} onClick={e => e.stopPropagation()}>
