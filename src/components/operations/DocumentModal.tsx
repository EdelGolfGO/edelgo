...
function SmartDateInput({ value, onChange, label }: { value: string; onChange: (val: string) => void; label: string }) {
  ...
  const segStyle = { background: "#23282E", border: "none", color: "#fff", fontSize: "13px", fontFamily: "'Barlow', sans-serif", outline: "none", textAlign: "center" as const, padding: "9px 4px" }

  return (
    <div style={{ position: "relative" }}>
      <label style={labelStyle}>{label}</label>
      <div style={{ display: "flex", alignItems: "center", background: "#23282E", border: "0.5px solid rgba(255,255,255,0.12)" }}>
        ...
        <span style={{ color: "#787E87", userSelect: "none" as const }}>/</span>
        ...
        <button type="button" onClick={() => setShowPicker(p => !p)} style={{ background: "none", border: "none", color: "#8B919A", cursor: "pointer", padding: "6px 10px", display: "flex", alignItems: "center" }}>
          <Calendar size={15} color={showPicker ? "#A91E22" : "#8B919A"} />
        </button>
      </div>
      {showPicker && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, zIndex: 400, background: "#2B3038", border: "0.5px solid rgba(255,255,255,0.12)", borderTop: "2px solid #A91E22", width: "260px", padding: "12px", boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" }}>
            <button type="button" onClick={() => { if (calMonth === 0) { setCalMonth(11); setCalYear(y => y-1) } else setCalMonth(m => m-1) }} style={{ background: "none", border: "none", color: "#B5BAC2", cursor: "pointer", fontSize: "16px", padding: "2px 6px" }}>‹</button>
            ...
            <button type="button" onClick={() => { if (calMonth === 11) { setCalMonth(0); setCalYear(y => y+1) } else setCalMonth(m => m+1) }} style={{ background: "none", border: "none", color: "#B5BAC2", cursor: "pointer", fontSize: "16px", padding: "2px 6px" }}>›</button>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", marginBottom: "4px" }}>
            {["Su","Mo","Tu","We","Th","Fr","Sa"].map(d => <div key={d} style={{ textAlign: "center", fontFamily: "'Barlow Condensed', sans-serif", fontSize: "9px", fontWeight: 700, letterSpacing: "0.1em", color: "#787E87", padding: "2px 0" }}>{d}</div>)}
          </div>
          ...
          <div style={{ marginTop: "10px", paddingTop: "10px", borderTop: "0.5px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between" }}>
            ...
            <button type="button" onClick={() => setShowPicker(false)} style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#8B919A", background: "none", border: "none", cursor: "pointer" }}>Close</button>
          </div>
        </div>
      )}
    </div>
      ...
      <div style={{ background: "#2B3038", border: "0.5px solid rgba(255,255,255,0.10)", borderTop: `2px solid ${config.eyebrowColor}`, width: "100%", maxWidth: step === "items" && config.showFactoryDropdown ? "960px" : "680px", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", padding: "20px 24px", borderBottom: "0.5px solid rgba(255,255,255,0.08)", background: "#20242A", position: "sticky", top: 0, zIndex: 10 }}>
          <div>
            ...
            <p style={{ fontSize: "12px", color: "#8B919A", fontFamily: "'Barlow', sans-serif", margin: 0, fontWeight: 400, textTransform: "none", letterSpacing: "normal" }}>{config.description}</p>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: "#8B919A", cursor: "pointer", padding: "4px", flexShrink: 0 }}><X size={20} /></button>
        </div>

        {/* Step tabs */}
        <div style={{ display: "flex", borderBottom: "0.5px solid rgba(255,255,255,0.08)", background: "#20242A" }}>
          {(["details", "items", "upload"] as const).map((s, i) => (
            <button key={s} onClick={() => setStep(s)} style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "11px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "10px 20px", cursor: "pointer", border: "none", background: "transparent", color: step === s ? "#fff" : "#8B919A", borderBottom: step === s ? `2px solid ${config.eyebrowColor}` : "2px solid transparent", marginBottom: "-1px" }}>
        ...
        <div style={{ padding: "24px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              {config.showLinkedPO && (
                <div>
                  ...
                  <p style={{ fontSize: "11px", color: "#787E87", fontFamily: "'Barlow', sans-serif", marginTop: "4px" }}>Enter the PO number this invoice is tied to</p>
                </div>
              )}
              ...
              {form.total_amount && parseFloat(form.total_amount) > 0 && (
                <div style={{ background: "#23282E", border: `0.5px solid ${config.eyebrowColor}30`, padding: "16px" }}>
                  <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "#8B919A", marginBottom: "12px" }}>Payment Schedule — {type === "distributor_invoice" ? "Money In" : "Money Out"}</p>
                  {[
                    ...
                    <div key={p.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                      <span style={{ fontSize: "12px", color: "#B5BAC2", fontFamily: "'Barlow', sans-serif" }}>{p.label}</span>
                  ...
                  <div style={{ marginTop: "10px", paddingTop: "10px", borderTop: "0.5px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontSize: "12px", color: "#9BA0A8", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 700 }}>Total</span>
          ...
          {step === "upload" && (
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              <label style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "12px", background: uploading ? "rgba(169,30,34,0.05)" : "#23282E", border: `1px dashed ${uploading ? "#A91E22" : "rgba(255,255,255,0.15)"}`, padding: "40px 20px", cursor: uploading ? "not-allowed" : "pointer", textAlign: "center", transition: "all 0.2s" }}>
                ...
                <Upload size={28} color={uploading ? "#A91E22" : "#787E87"} />
                <div>
                  <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "14px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: uploading ? "#A91E22" : "#9BA0A8", margin: "0 0 4px" }}>{uploading ? "Uploading..." : "Click to Upload"}</p>
                  <p style={{ fontSize: "12px", color: "#787E87", fontFamily: "'Barlow', sans-serif", margin: 0 }}>PDF, PNG, JPG, Excel — up to 20MB each</p>
                </div>
              </label>

              {uploadedFiles.length > 0 && (
                <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                  <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#8B919A", margin: "0 0 4px" }}>Documents ({uploadedFiles.length})</p>
                  {uploadedFiles.map((file, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#23282E", border: `0.5px solid ${file.url ? "rgba(90,158,90,0.3)" : "rgba(255,255,255,0.08)"}`, padding: "10px 14px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", flex: 1 }}>
                        <FileText size={14} color={file.url ? "#5A9E5A" : "#8B919A"} />
                        <span style={{ fontSize: "13px", color: "#E0E2E6", fontFamily: "'Barlow', sans-serif" }}>{file.name}</span>
                      ...
                      <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                        ...
                        <button onClick={() => setUploadedFiles(prev => prev.filter((_, idx) => idx !== i))} style={{ background: "none", border: "none", color: "#787E87", cursor: "pointer" }}><X size={14} /></button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{ display: "flex", gap: "10px", marginTop: "8px" }}>
                <button onClick={() => setStep("items")} style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "12px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#9BA0A8", background: "transparent", border: "1px solid #666C75", padding: "10px 20px", cursor: "pointer" }}>← Back</button>
                <button onClick={handleSave} disabled={saving || uploading} style={{ flex: 1, fontFamily: "'Barlow Condensed', sans-serif", fontSize: "13px", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#fff", background: saving || uploading ? "#666C75" : config.eyebrowColor, border: "none", padding: "12px", cursor: saving || uploading ? "not-allowed" : "pointer" }}>
